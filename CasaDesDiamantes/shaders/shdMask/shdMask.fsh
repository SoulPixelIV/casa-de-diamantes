...
void main() {
	...
	float rampPos = smoothstep(maskVal, maskVal + 0.14, timeExt);
	vec4 rampCol = texture2D(rampTex, vec2(rampXMin + rampXMax * rampPos, rampYMin));
	
	// Mix RGBA
	float blendWeight = smoothstep(maskVal + 0.14 * 0.25, maskVal + 0.14, timeExt);
	...
	baseCol.a *= smoothstep(maskVal, maskVal + 0.09, timeExt);
