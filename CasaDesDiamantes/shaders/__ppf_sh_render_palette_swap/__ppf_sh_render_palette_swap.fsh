...
vec3 palette_swap_fx(vec3 color) {
	...
	vec2 uv2 = vec2(u_palette_swap_flip > 0.5 ? 1.0-lum : lum, 1.0-ceil(1.0+u_palette_swap_row)/u_palette_swap_height);
	vec3 col_pal = texture2D(u_palette_swap_tex, uv2).rgb;
	
	if (length(_col - col_pal.rgb) >= u_palette_swap_threshold) {
		_col = col_pal;
	}
	return _col;
}
