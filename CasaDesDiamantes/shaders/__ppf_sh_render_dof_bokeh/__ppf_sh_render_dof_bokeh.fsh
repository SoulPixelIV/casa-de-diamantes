...
vec4 dof_fx(vec2 uv, vec2 fpos) {
	float center_depth = texture2D(u_coc_tex, uv).r;
	float center_size = center_depth * u_bokeh_radius;
	float total = 1.0;
	...
	for(float ang = 0.0; ang < 360.0; ang += Golden_Angle) {
		...
		float depth = texture2D(u_coc_tex, uv2).r;
		float coc = depth * u_bokeh_radius;
