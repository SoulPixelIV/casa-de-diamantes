...
if (key_escape && !pauseDelayStart && hp > 0 && !dialogueSystem_obj.inCutscene && camera_obj.blackscreenStrength < 0.1 && !deathActivated && !camera_obj.cameraTrainCutscene && room != level28 && (movement || sittingInMinecart)) {
	save_scr();
...
if (movement && !isZombie)
{
	...
	if (huggingWall)
	{
		...
		if (player_obj.inputMethod != 0) {
			if (pad != -1) {
				gamepad_set_vibration(pad, 0.1, 0.1);
			}
		}
...
{	
	...
	if (global.syringes < 1 || room == level10 || room == level25 || room == level26 || room == level27 || room == level28)
	{
