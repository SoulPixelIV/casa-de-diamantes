...
if (key_escape && !pauseDelayStart && hp > 0 && !dialogueSystem_obj.inCutscene && !inCutscene && camera_obj.blackscreenStrength < 0.1 && !deathActivated) {
	save_scr();
			...
			if (global.currentWeapon == pickedWeapon.pistol && global.pistolCooldown <= 0)
			{
				with (player_obj)
				{
					...
					if (key_shoot && !reloading)
					{
						...
						{
							if (room == level_Casino || room == level_CasinoRoof || room == level_DiningHall) {
